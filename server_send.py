...
def main(watermark: int, log_file: str, client_ep: int, enc_lst: list, server_client_split: float, force_data_gen: bool, server_ep :int =150):
	...
	os.system(f'echo {correct_key} > raw_data.txt')
	os.system('openssl pkeyutl -encrypt -inkey rpi_public.pem -pubin -in raw_data.txt -out enc_data.enc')
	print('Encrypted Nk with Pk!')

	# Send Enc key to Server
	with open("enc_data.enc", "rb") as f:
			data = f.read()
	s.send(data)
	f.close()
	s.close()
	print('Sent Encrypted NN key to Client!')
	time.sleep(1)

	model_1 = pickle.dumps(enc_model)
	model_1_compressed = gzip.compress(model_1)

	# Send Enc model to Server
	print("ML Socket connection")
	with MLSocket() as mls:
			mls.connect(('10.42.0.60', 54321))
			mls.send(model_1_compressed)
	print("Sent Encrypted model to Client")
	mls.close()
