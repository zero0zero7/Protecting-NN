...
def read_log(file_path = '/hdd/projects/XY_FYP/new/combine/logs/fashion_nnlock_textoverlay2.log'):
	...
	with open(file_path, 'r') as fr:
		...
		if line.startswith("{"):
			line = line.rstrip("\n")
			original = ast.literal_eval(line)
		...
		if line.startswith("{"):
			line = line.rstrip("\n")
			incremental = ast.literal_eval(line)
	...
	context["wm_diff"] = round(context["decrypted wm_acc"] - context["quantized wm_acc"], 5)
	
	return (context, original, incremental)
...
def main(file_header = "fashion_nnlock_textoverlay"):
	...
	log_name = f"{file_header}*.log"
	log_path = os.path.abspath(os.path.join("../logs/", log_name))
	matched_files = glob.glob(log_path)

	for f in matched_files:
		print(f)
		context, original, incremental = read_log(f)
		if "non-locked parameters" not in context.keys():
			print("NOOO")
