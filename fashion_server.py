...
def main(watermark: int, log_file: str, client_ep: int, enc_lst: list, server_client_split: float, force_data_gen: bool, server_ep :int =150):
	...
	model =  server_models.fashion_nnlock(image_shape)
	pat = model_utils.patience(server_ep)
	lr_sch = ExponentialDecay(initial_learning_rate=0.01, decay_rate=0.1/server_ep, decay_steps=100000)
	...
	la.write("\nmodel accuracy trigger: {:.5f}".format(score[1]*100))
	print("\nquantized_copy")
	quantized_copy = model_utils.copy_part(quantized_model, 'quant_fashion_'+trig_type+'.weights.h5')
	# Generate NN Key
	...
	client_opt = SGD(learning_rate=lr_sch, nesterov=False, clipnorm=1.0, clipvalue=0.5)
	...
	trained = model_utils.copy_part(enc_model, 'fashion_'+trig_type+'_trained.weights.h5')
	la.write("\n==========\nadditional training: ")
	...
	print("\n=========DECRYPT TRAINED=======")
	decr_trained = model_utils.dec_layers(trained, enc_lst, correct_key, 'fashion_'+trig_type+'_decr.weights.h5')
	la.write("\n==========\ndecrypted: ")
	print("TEST")
